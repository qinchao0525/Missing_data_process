...
Usage: Gray prediction.
Auther: Qinchao
Latest modfiying time:2016/11/25
***********************************************/
#ifndef GRAYPRE_H_
#define GRAYPRE_H_
# include <vector>
...
输入：
double a[] ------ 灰度预测的前期数据集
int a_num ------ 前期数据集中数据的个数
输出：
灰度预测的输出结果
*/
double GM_returnk_1(double a[], int a_num)
{
	std::vector<double> X0(a_num);
	std::vector<double> X1(a_num);
	//X0,X1
	...
	for (int i_1 = 0; i_1<a_num; i_1++)
		X1[i_1] = sum(a, i_1);
	//计算B
	std::vector<std::vector<double> > B(a_num - 1);
	...
	//计算Yn
	std::vector<double> Yn(a_num - 1);
	for (int i_4 = 0; i_4<a_num - 1; i_4++)
		Yn[i_4] = X0[i_4 + 1];
	//计算a，u
	//Bt和B相乘的结果
	std::vector<std::vector<double> > BB(2);
	...
	//计算Bt和B相乘的逆
	std::vector<std::vector<double> > B_(2);
	...
	B_[1][1] = BB[0][0] / BB_z;
	//B_此时为逆矩阵，计算B_ * Bt
	std::vector<std::vector<double> > BBb(2);
